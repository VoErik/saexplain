...
def load_mae_for_finetuning(
        num_classes: int,
        architecture: str = "vit_huge_patch14_224",
        device: str = "cuda"
):
    """
    Loads a MAE-pretrained ViT and replaces its head for fine-tuning.
    """
    print(f"Loading pretrained MAE ({architecture}) for fine-tuning.")
    model = timm.create_model(architecture, pretrained=True, num_classes=num_classes)

    model.to(device)
    print("MAE model loaded and configured for fine-tuning.")
    return model
...
def fine_tune_mae(
    ...
    param_groups = [
        ...
        {"params": model.patch_embed.parameters(), "lr": encoder_lr},
        {"params": model.blocks.parameters(), "lr": encoder_lr},
        {"params": model.norm.parameters(), "lr": encoder_lr}
    ]
...
if __name__ == '__main__':
    ...
    for labelkey in labelkeys:
        for seed in seeds:
            ...
            wandb.init(
                project="mae-finetune-fitzpatrick17k",
                config={
            ...
            mae_model = load_mae_for_finetuning(
                num_classes=len(class_names),
                architecture=wandb.config.model,
                device=device
            )
