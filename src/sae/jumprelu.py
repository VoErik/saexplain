...
@dataclass
class JumpReLUSAETrainingConfig(TrainingSAEConfig):
...
class JumpReLUSAE(SAE[JumpReLUSAEConfig]):
    ...
    def encode(
        ...
        base_acts = self.activation_fn(hidden_pre_activations)

        # 2) Zero out any unit whose (hidden_pre <= threshold).
        #    We cast the boolean mask to the same dtype for safe multiplication.
        jump_relu_mask = (hidden_pre_activations > self.threshold).to(base_acts.dtype)
        features = base_acts * jump_relu_mask
...
class JumpReLUTrainingSAE(TrainingSAE[JumpReLUSAETrainingConfig]):
    """
    ...
    def __init__(self, cfg: JumpReLUSAETrainingConfig, use_error_term: bool = False):
        super().__init__(cfg)
    ...
    def encode_with_hidden_pre(
        ...
        features = JumpReLU.apply(hidden_pre_activations, self.threshold, self.bandwidth)

        return features, hidden_pre_activations  # type: ignore
